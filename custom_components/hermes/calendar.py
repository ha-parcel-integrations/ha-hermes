"""Calendar platform for the Hermes parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the Hermes deliveries calendar from a config entry."""
    async_add_entities([HermesDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class HermesDeliveriesCalendar(CoordinatorEntity[HermesCoordinator], CalendarEntity):
    """A read-only calendar of expected Hermes deliveries.
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "Hermes parcel")
            description_parts = [
