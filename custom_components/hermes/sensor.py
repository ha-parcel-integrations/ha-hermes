"""Sensor platform for the Hermes parcel tracker integration."""
...
async def async_setup_entry(
    ...
    entities: list[SensorEntity] = [
        HermesIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
        HermesAwaitingPickupSensor(coordinator, entry),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            HermesParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(HermesNextDeliverySensor(coordinator, entry))
    entities.append(HermesDeliveredParcelsSensor(coordinator, entry))
    entities.append(HermesLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class HermesAwaitingPickupSensor(
...
class HermesIncomingParcelsSensor(
    CoordinatorEntity[HermesCoordinator], SensorEntity
):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                HermesParcelSensor(self.coordinator, self._entry, barcode)
...
class HermesParcelSensor(CoordinatorEntity[HermesCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Hermes parcel."""
    ...
    def __init__(
        self, coordinator: HermesCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class HermesNextDeliverySensor(
    CoordinatorEntity[HermesCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: HermesCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class HermesDeliveredParcelsSensor(
    CoordinatorEntity[HermesCoordinator], SensorEntity
):
    """Recently delivered tracked Hermes parcels."""
...
class HermesLastUpdateSensor(
    CoordinatorEntity[HermesCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Hermes was last polled successfully."""
